...
// ---------------------------------------------------------------
// A third axis: how the name is REACHED
// ---------------------------------------------------------------
//
// Carrier × exit covers "how the value is built" and "how it leaves".
// Both of the correctness bugs this corpus found last were on a third
// axis neither one represents: the name is reached under a COMPUTED
// KEY whose value arrives as a string.
//
//   const payload = { alphaTop: 1 };
//   const echoed = (payload as any)["alphaTop"];
//
// `alphaTop` is a static key at the declaration and a string VALUE at
// the read. Renaming only the first makes the read `undefined` — a
// wrong answer, not a crash, and nothing about the exit is involved:
// these cases take the `none` exit and still have to hold.
//
// The receiver shape is what varies, because that is where the holes
// were. A bare binding has a symbol the observability phases can
// follow; `this`, a field, a property chain, a call result and an array
// element do not, and each was handled separately (or not at all).
const DYNAMIC_RECEIVERS = {
  binding: {
    what: "a top-level binding",
    decl: () =>
      [
        `const payload = { ${TOP}: 1 };`,
        `function pick(k: string): number | undefined {`,
        `  return (payload as any)[k];`,
        `}`,
      ].join("\n"),
  },
  thisCast: {
    what: "`this` inside a method, cast to `any`",
    decl: () =>
      [
        `class Holder {`,
        `  ${TOP}: number = 1;`,
        `  pick(k: string): number | undefined {`,
        `    return (this as any)[k];`,
        `  }`,
        `}`,
        `const holder = new Holder();`,
        `const pick = (k: string): number | undefined => holder.pick(k);`,
      ].join("\n"),
  },
  privateField: {
    what: "a `#private` field holding an object with named keys",
    decl: () =>
      [
        `class Holder {`,
        `  #bag: { ${TOP}: number } = { ${TOP}: 1 };`,
        `  pick(k: string): number | undefined {`,
        `    return (this.#bag as any)[k];`,
        `  }`,
        `}`,
        `const holder = new Holder();`,
        `const pick = (k: string): number | undefined => holder.pick(k);`,
  ...
  publicField: {
    what: "a public field holding an object with named keys",
    decl: () =>
      [
        `class Holder {`,
        `  bag: { ${TOP}: number } = { ${TOP}: 1 };`,
  ...
  propChain: {
    what: "a property chain read from outside the class",
    decl: () =>
      [
        `class Holder {`,
        `  bag: { ${TOP}: number } = { ${TOP}: 1 };`,
  ...
  callResult: {
    what: "the result of a call, indexed directly",
    decl: () =>
      [
        ...
        `}`,
        `function pick(k: string): number | undefined {`,
        `  return (build() as any)[k];`,
        `}`,
      ].join("\n"),
  },
  arrayElement: {
    what: "an element of an array, indexed by a numeric then a string key",
    decl: () =>
      [
        `const list = [{ ${TOP}: 1 }];`,
        `function pick(k: string): number | undefined {`,
        `  return (list[0] as any)[k];`,
        `}`,
      ].join("\n"),
  },
};
...
function renderDynamicCase(shapeKey) {
  const shape = DYNAMIC_RECEIVERS[shapeKey];
  const lines = [];
  lines.push(`// GENERATED by scripts/generate_mangle_cases.mjs — do not edit.`);
  lines.push(`//`);
  lines.push(`// Reach:    a computed key whose value is the string "${TOP}".`);
  lines.push(`// Receiver: ${shape.what}.`);
  lines.push(`//`);
  lines.push(`// Derived expectation: keep [${TOP}]. The name is a static key at`);
  lines.push(`// the declaration and a string VALUE at the read, so renaming one`);
  lines.push(`// without the other turns \`echoed\` into \`undefined\`. Nothing is`);
  lines.push(`// manglable: the key set behind a computed read is unbounded, so`);
  lines.push(`// the reservation is the wildcard and ${CTL} rides along.`);
  lines.push("");
  lines.push(shape.decl());
  lines.push("");
  lines.push(`export const echoed = pick("${TOP}");`);
  lines.push("");
  lines.push(`// The control: same kind of value, never reached dynamically.`);
  lines.push(`const control = { ${CTL}: 7 };`);
  lines.push(`export const total = control.${CTL};`);
  lines.push("");

  const kebab = (t) => t.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`);
  const meta = {
    origin: "generated — scripts/generate_mangle_cases.mjs",
    generated: true,
    what: `A property reached under a computed key, off ${shape.what}. The name is a static key at the declaration and a string value at the read; renaming one without the other is a wrong answer, and no exit is involved.`,
    exports: ["echoed", "total"],
    expectKeep: [TOP],
    expectMangle: [],
    expectStatus: "pass",
  };
  ...
  return {
    name: `dynkey-${kebab(shapeKey).replace(/^-/, "")}`,
    files: {
      "index.ts": lines.join("\n"),
      "case.json": `${JSON.stringify(meta, null, 2)}\n`,
      "driver.mjs": driver,
...
function allCases() {
  ...
  for (const shapeKey of Object.keys(DYNAMIC_RECEIVERS)) {
    out.push(renderDynamicCase(shapeKey));
  }
  return out;
}
...
console.log(
  `generate_mangle_cases: wrote ${cases.length} cases (${Object.keys(CARRIERS).length} carriers × ${Object.keys(EXITS).length} exits, plus ${Object.keys(DYNAMIC_RECEIVERS).length} computed-key receiver shapes) to fixtures/mangle-safety/generated/`,
);
