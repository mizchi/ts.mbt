...
| **`as const` on an OBJECT LITERAL** (`const a = { k: "hello" } as const`, `a.k` against `"other"`) | **BLIND — with a measured cause, see §3.** The member widens to `string`, so `is_widening_direction_mismatch` correctly suppresses it |
| `as const` elsewhere (`"hello" as const`, a tuple index, a DECLARED literal member) | CAUGHT at the verdict; the message names `string` where tsc names `"hello"` (a single `Literal` widens, a UNION of literals does not) |
...
|---|---|---|
| `const a = { k: "hello" } as const; const r: "other" = a.k` | TS2322 | **The suppression is RIGHT and the widening is the bug.** `is_widening_direction_mismatch` drops an `expected "X" but got string` mismatch because OUR inference sometimes widens where tsc keeps a literal — so a report there can be about our gap rather than the program (batch EL kept that arm for exactly this reason). Here it fires because `as const` does not keep its members' literal types: `a.k` comes out `string`, where tsc has `"hello"`. Measured 2026-09-18 — the DECLARED spelling (`declare const o: { k: "hello" }`) reports, so the member READ is not what loses it, and `a.nope` proves the member is `string` rather than `Any`. The fix is in `as const`, not in the suppression; `src/transform`'s `as_const_inline` already models the construct on its own side |
