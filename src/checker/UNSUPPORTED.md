...
| **a `string` source against a string-LITERAL target** (`declare const d: { k: string }`, `d.k` against `"other"`) | **BLIND — see §3.** `as const` is a red herring: the identical shape without it is equally silent |
...
|---|---|---|
| `declare const d: { k: string }; const r: "other" = d.k` | TS2322 | A `string` source against a string-LITERAL target is silent, and the cause is **NOT yet localized** — this row records a measurement, not a diagnosis. The first draft blamed `is_widening_direction_mismatch` and was wrong twice over: batch EL REMOVED that function's string arm (only number / boolean / bigint / void remain), and `as const` is not the discriminator either. A six-cell matrix says what is: a LITERAL member against a literal target reports (`{ k: "hello" }`), a `string` member against a literal target does not, and both report against `number` — so it is the TARGET being a string literal that suppresses, whatever the receiver's origin. `is_assignable_to(String_, Literal("foo"))` is correctly `false` (asserted in `assignability_wbtest.mbt`) and the `(String_, Literal(_))` arm at `expr_check.mbt:7950` is an OVERLAP predicate for `==`, correct as written, so the suppression is a third site — the applied-in-some-places family again. Worth taking with the corpus as the gate, since a `string`-to-literal report is the commonest real TS2322 there is |
