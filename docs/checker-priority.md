...
> ## 2026-09-01: 下の「結論」は棄却された（測定による）
>
> このドキュメントの結論は **「増分的な sound recall win は枯渇。668→700 は
> 小さな pass の積み上げでは到達しない」** だった。これは **TS6 オラクル時代
> （2026-06-26）の観測**であり、現在の TS7 オラクルには当てはまらない。
>
> 反例は 1 コマンドで出る。`scripts/checker_miss_buckets.mjs` は MISS を
> baseline のエラーコードで分類する（オラクル本体は FP をゲートするだけで、
> 「MISS 397」は何の順位付けにもならない — 数を報告することと、次に何を作るかを
> 言えることは別物だった）。答え:
>
> - MISS 397 件のうち **128 件が TS1xxx（純粋な文法規則）で反転可能**。
>   型機構・flow narrowing・generic instantiation は一切不要。
> - うち **91 件は baseline のエラーコードが全て TS1xxx** — つまり型判断が
>   1 つも要らない。
>
> 実証として batch BU（strict mode / `with` / label）は **9 ファイル**を
> +9 TP、FP 0 で獲得した。1 つは 4 つの綴りのうち 2 つにしか規則が書かれて
> いなかった既存バグ（`eval++` が無検査）で、recall の話である前に
> **バグだった**。
>
> したがって「腰を据えた大機能を 1 つずつ」は **今の正しい次手ではない**。
> 安く・低 FP リスクで・独立に検証できる文法規則が 3 桁残っている。
> 大機能（代入可能性・flow narrowing）の評価そのものは下の記述が今も有効で、
> **順番だけ**が変わる。
>
> 教訓は 3 度目のもので、この repo の他の場所にも書かれている:
> **ラベルや過去の結論ではなく、目的関数を測る。** 「枯渇」は
> 「TS6 オラクルで、当時のツールで見えた範囲では枯渇」の略記だった。
>
> 作成: 2026-06-26 / 当時の基準: pinned conformance gate `recall=668/815`,
> `precision=414/414 (0 FP)`（TS6）。現在: TS7 オラクルで
> **TP 2390 / MISS 344 / FP 0 / PFLEGAL 0 / TN 1750**。
>
> ### 2026-09-02 追記: 結論は 2 つ目の意味でも誤っていた
>
> 上の反証は「安い文法規則がまだ 3 桁ある」だった。それとは独立に、
> **「大機能が残っているだけ」という前提自体が成り立っていない。**
>
> MISS 上位 4 バケット — TS2322 (30 件) / TS2345 (20) / TS2339 (18) /
> TS2304 (15)、実世界で最も遭遇する 4 つ — の**最も単純な綴りは、すべて
> 既に検出できている**。1 コマンドで確認できる:
>
> - `const x: string = 1` / `function f(): string { return 1 }` /
>   `let x: string = "a"; x = 1` / `class C { x: string = 1 }` /
>   `const x: string[] = [1]` / `const x: {a: string} = {a: 1}` — 代入 6 箇所
> - `f(1)` に対する `f(a: string)`、引数過多、メソッド呼び出し — 3 箇所
> - `{a:1}` への `o.b`、注釈付き、クラス、`string` — 4 箇所
> - `nonexistent;` / `nonexistent()` / `const x = nonexistent` / 型参照
>
> すべて 1 issue を報告する。つまりこれらの MISS は「機構が無い」ではなく
> **「機構はあり、そのファイルの何か特定の要素に負けている」**。実際に
> ファイルを開くと、conditional/mapped/template-literal 型、index signature
> の相互作用、`this` 型、`#private`、`globalThis` といった具体的な形に
> 分かれる。「代入可能性を実装する」という粒度のタスクは存在しない。
>
> もう 1 つ、順位付けの単位についての訂正。バケット表は (file, code) を
> 数えるが、反転するのは **file** である（1 つでも検出できれば TP）。
> `checker_miss_buckets.mjs` は `solo`（そのコードだけが手がかりのファイル
> 数 = 保証収量）と貪欲被覆を出すようになった。上位 12 規則で MISS の
> 33% にしか届かない — 尾が本当に長い。
>
> そしてコードによる被覆は**機能クラスタを見落とす**: `symbolProperty*` は
> TS2403/2420/2353/2454/2464/2320/2554 の 7 コードに散っていて実体は 1 機能。
> ディレクトリで括ると `parser/ecmascript5` 49 件が最大かつ最安だが、中身は
> `parserErrorRecovery_ParameterList6` のような壊れた構文で、**実コードでは
> 遭遇しない**。コーパス件数を遭遇頻度の代理にするのは、この repo が繰り返し
> 記録している「ラベルが目的にすり替わる」失敗そのもの。
>
> 最後に、エラーコードは**収量**のクラスでもない。batch BZ は TS2420
> （5 件 / solo 3）から見積もって着手し、実際に転んだのは **1 件**だった。
> 残り 4 件は同じコードを**まったく別の理由**で出している — interface
> マージ経由の private 非互換、overload の代入可能性、数値インデクサ。
> バケット件数からバッチの価値を見積もるには、結局ファイルを開く必要がある。
> それでも入れる理由はコーパスが示せない側にある: interface にメンバを足して
> 実装を忘れるのは人間が実際にやることで、型システムを網羅するために書かれた
> conformance スイートにはその形がほとんど無い。
>
